...
from hita.tokenizer.tokenizer_image.vq_loss import VQLoss
...
def main(args):
    ...
    vq_loss = VQLoss(
        ...
        enable_vfm_recon = args.enable_vfm_recon,
    ).to(device)
        ...
        with tqdm(loader, dynamic_ncols=True, disable=not is_main_process()) as train_dl:
            for i, (imgs, labels,) in enumerate(train_dl):
                ...
                with torch.cuda.amp.autocast(dtype=ptdtype):  
                    recons, codebook_loss, (z_indices, q_indices) = vq_model(imgs)

                    loss_gen, sim_loss = vq_loss(codebook_loss, imgs, recons, optimizer_idx=0, global_step=train_steps+1, 
                ...
                with torch.cuda.amp.autocast(dtype=ptdtype):
                    loss_disc = vq_loss(codebook_loss, imgs, recons, optimizer_idx=1, global_step=train_steps+1,
                                        log_every=args.log_every)
                ...
                img_indices = torch.unique(concat_all_gather(z_indices))
                slot_indices = torch.unique(concat_all_gather(q_indices))
                usage_img, usage_slot = img_indices.size(0) / args.codebook_size, slot_indices.size(0) / args.codebook_size
                
                if (train_steps % args.log_every == 0) & is_main_process():
                    
                    if args.enable_vfm_recon:
                ...
                if (train_steps > 0) & (train_steps % args.ckpt_every == 0) & is_main_process():
                    ...
                    if args.ema:
                        ...
                        checkpoint["ema"] = {k:v for k,v in ema.state_dict().items() if 'backbone' not in k}
                        
                    ckpt_path = osp.join(checkpoint_dir, 'hita_vqgan_step_{}.pt'.format(train_steps))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--enable-vfm-recon", action='store_true', default=False)
